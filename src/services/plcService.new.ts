...
class PLCService extends EventEmitter {
  ...
  public async connect(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      // Configuración mejorada para S7-400
      const connectionParams = {
        port: 102, // Puerto estándar para S7 TCP/IP
        host: this.config.ip,
        rack: this.config.rack,
        slot: this.config.slot,
        timeout: 15000, // Aumentamos el timeout para S7-400
        localTSAP: '0100', // TSAP local personalizado
        remoteTSAP: `01${this.config.rack.toString(16).padStart(2, '0')}${this.config.slot.toString(16).padStart(2, '0')}`, // TSAP remoto basado en rack/slot
        connectionType: 'PG', // Tipo de conexión: PG (programadora)
        maxRetries: 3, // Número máximo de reintentos
        doNotOptimize: true // Desactivar optimizaciones que pueden causar problemas
      };

      logger.info(`Intentando conectar al PLC S7-400 en ${this.config.ip}:${connectionParams.port}`);
      logger.info(`Configuración: Rack=${this.config.rack}, Slot=${this.config.slot}, localTSAP=${connectionParams.localTSAP}, remoteTSAP=${connectionParams.remoteTSAP}`);
      
      this.client.initiateConnection(connectionParams, (err: Error) => {
        if (err) {
        ...
        logger.info(`Configuración: Rack=${this.config.rack}, Slot=${this.config.slot}`);
        this.connected = true;
        ...
        try {
          ...
          logger.info(`Se han registrado ${Object.values(this.variables).length} variables para monitoreo`);
  ...
  public async readAllItems(): Promise<Record<string, any>> {
    ...
    try {
      ...
      return new Promise<Record<string, any>>((resolve, reject) => {
        ...
        // Intentar leer valores individuales en lugar de readAllItems
        this.client.readItems(variablesToRead, (err: Error, values: Record<string, any>) => {
  ...
  public writeItem(address: string, value: any): boolean {
    ...
    try {
      logger.info(`Intentando escribir en ${address} el valor ${value}`);
      
      // Escribir el valor en el PLC
      this.client.writeItems(address, value, (err: Error) => {
        if (err) {
          logger.error(`Error al escribir en ${address}:`, err);
          return false;
        }
        logger.info(`Valor escrito correctamente en ${address}: ${value}`);
        return true;
      });
      
      // Si llegamos aquí, consideramos que la operación fue exitosa
      // (la callback se ejecutará de forma asíncrona)
      return true;
    } catch (error) {
      logger.error(`Error al escribir en ${address}:`, error);
      return false;
    }
  }
